        ...
        if await is_maintenance() is False:
            if message.from_user.id not in SUDOERS:
                return await message.reply_text(
                    text=f"{app.mention} Baxımdadır, ziyarət edin <a href={SUPPORT_CHAT}>Dəstək söhbəti</a> Səbəbini bilmək üçün.",
                ...
                if not admins:
                    ...
                    if message.from_user.id not in admins:
                        if await is_skipmode(message.chat.id):
                            ...
                            text = f"""<b>Admin hüquqları lazımdır</b>

vasitəsilə admin keşini yeniləyin : /reload

» {upvote} Bu hərəkəti yerinə yetirmək üçün lazım olan səslər"""
        ...
        if await is_maintenance() is False:
            if CallbackQuery.from_user.id not in SUDOERS:
                return await CallbackQuery.answer(
                    f"{app.mention} baxım altındadır, səbəbini bilmək üçün dəstək söhbətinə baş çəkin.",
                    show_alert=True,
                )
        try:
